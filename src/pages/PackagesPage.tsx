...
import { Check, ShieldCheck, Calendar, Users, Clock, Video, Star, Flame } from 'lucide-react';
...
const PackagesPage = () => {
  ...
  // Generate random recent order counts for each package
  const getRecentOrders = (packageId: string) => {
    const randomOrders = Math.floor(Math.random() * 50) + 20; // Random number between 20-70
    return randomOrders;
  };
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    <div className="inline-block text-sm font-medium px-3 py-1 rounded-full bg-accent/50 text-accent-foreground">
                      {pkg.duration}
                    </div>
                    <div className="flex items-center text-sm text-rose-500 font-medium">
                      <Flame className="h-4 w-4 mr-1 animate-pulse" />
                      {getRecentOrders(pkg.id)} orders this week
                    </div>
                  </div>
